    ...
    try {
      ...
      const botReply = data.reply || (
        language === 'ko'
          ...
          : 'Failed to retrieve response. Please try again.'
      );
      ...
      const fallbackErr =
        language === 'ko'
          ? `일시적인 통신 오류가 발생했습니다. 직접 문의를 원하시면 [ ${email} ]으로 메일을 보내주시면 신속하게 답변드리겠습니다.`
          : language === 'ja'
          ? `一時的な通信エラーが発生しました。直接のお問い合わせは公式メール [ ${email} ] までお願いいたします。`
          : `A temporary network issue occurred. For direct inquiries, please email [ ${email} ].`;
      setMessages((prev) => [
        ...prev,
        {
          id: `bot-${Date.now()}`,
