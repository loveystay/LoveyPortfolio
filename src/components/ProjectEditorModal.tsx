...
import { isYouTubeUrl, extractYouTubeId, getYouTubeThumbnailUrl, getYouTubeEmbedUrl } from '../utils/youtube';
import {
  X,
  Check,
  Film,
...
const PRESET_VIDEO_IMAGES = [
  { label: '테크/기기 리뷰', url: 'https://images.unsplash.com/photo-1550745165-9bc0b252726f?q=80&w=1200&auto=format&fit=crop' },
  { label: '감성 브이로그', url: 'https://images.unsplash.com/photo-1513519245088-0e12902e5a38?q=80&w=1200&auto=format&fit=crop' },
  { label: '패션/뷰티 숏폼', url: 'https://images.unsplash.com/photo-1509631179647-0177331693ae?q=80&w=1200&auto=format&fit=crop' },
  { label: '지식/경제 정보', url: 'https://images.unsplash.com/photo-1460925895917-afdab827c52f?q=80&w=1200&auto=format&fit=crop' },
];

const PRESET_PRODUCT_IMAGES = [
  { label: '뷰티/스킨케어 LP', url: 'https://images.unsplash.com/photo-1556228720-195a672e8a03?q=80&w=1200&auto=format&fit=crop' },
  { label: '프리미엄 텀블러 LP', url: 'https://images.unsplash.com/photo-1517256064527-09c73fc73e38?q=80&w=1200&auto=format&fit=crop' },
  { label: '주방/생활용품 LP', url: 'https://images.unsplash.com/photo-1556911220-e15b29be8c8f?q=80&w=1200&auto=format&fit=crop' },
  { label: '원두/F&B 식품 LP', url: 'https://images.unsplash.com/photo-1495474472287-4d71bcdd2085?q=80&w=1200&auto=format&fit=crop' },
  { label: '음향/테크 가전 LP', url: 'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?q=80&w=1200&auto=format&fit=crop' },
];

const PRODUCT_CATEGORIES = ['뷰티/화장품', '식품/F&B', '생활/가전', '패션/잡화', '헬스/건강식', '와디즈/텀블벅 펀딩', 'IT/테크'];
...
export const ProjectEditorModal: React.FC<ProjectEditorModalProps> = ({
  ...
  // Category state
  const [category, setCategory] = useState<'YOUTUBE VIDEO' | 'PRODUCT PAGE' | 'SHORTS / REELS'>('YOUTUBE VIDEO');
  ...
  const handleCategorySelect = (newCat: 'YOUTUBE VIDEO' | 'PRODUCT PAGE' | 'SHORTS / REELS') => {
    setCategory(newCat);
  ...
  useEffect(() => {
    if (projectToEdit) {
      ...
      const cat = projectToEdit.category === 'PRODUCT PAGE'
        ? 'PRODUCT PAGE'
      ...
      setRole(projectToEdit.role || (cat === 'PRODUCT PAGE' ? '상세페이지 디자인' : '영상 편집'));
      setDuration(projectToEdit.duration || '');
      setImage(projectToEdit.image || (cat === 'PRODUCT PAGE' ? PRESET_PRODUCT_IMAGES[0].url : PRESET_VIDEO_IMAGES[0].url));
      ...
      setTools(projectToEdit.tools || (cat === 'PRODUCT PAGE' ? ['Photoshop', 'Figma'] : ['Premiere Pro', 'Photoshop']));
  ...
  const handleRequestAISuggest = async (targetField?: 'description' | 'fullStory' | 'all') => {
    ...
    try {
      ...
      if (data && data.result) {
        if (targetField === 'description' && typeof data.result === 'string') {
          ...
          setAiSuccessMsg(isProductPage ? '상세페이지 요약 카피가 적용되었습니다!' : '영상 요약 문구가 적용되었습니다!');
        } else if (targetField === 'fullStory' && typeof data.result === 'string') {
          setFullStory(data.result);
          setAiSuccessMsg(isProductPage ? '상세페이지 기획/디자인 스토리가 적용되었습니다!' : '영상 편집 스토리가 적용되었습니다!');
  ...
  const handleApplyAllAI = () => {
    ...
    if (aiGeneratedResult.description) setDescription(aiGeneratedResult.description);
    if (aiGeneratedResult.fullStory) setFullStory(aiGeneratedResult.fullStory);
    if (aiGeneratedResult.highlights && aiGeneratedResult.highlights.length > 0) {
  ...
  const handleSectionFieldChange = (index: number, field: keyof DetailSection, value: any) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const projectData: Omit<Project, 'id'> = {
      ...
      categoryTag,
      year: year.trim() || '2024',
      client: client.trim() || (isProductPage ? '브랜드사 프로젝트' : '개인/채널 프로젝트'),
      role: role.trim() || (isProductPage ? '상세페이지 디자인' : '영상 컷편집 & 디자인'),
      duration: duration.trim() || undefined,
      image: image.trim() || (isProductPage ? PRESET_PRODUCT_IMAGES[0].url : PRESET_VIDEO_IMAGES[0].url),
      videoUrl: isProductPage ? undefined : (videoUrl.trim() || undefined),
      description: description.trim(),
      fullStory: fullStory.trim() || description.trim(),
      featuredInHome,
      tools: tools.length > 0 ? tools : (isProductPage ? ['Photoshop', 'Figma'] : ['Premiere Pro']),
      highlights: highlights.filter((h) => h.trim().length > 0),
      productCategory: isProductPage ? productCategory : undefined,
      longDetailImage: isProductPage ? (longDetailImage.trim() || undefined) : undefined,
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 overflow-y-auto">
        ...
        <motion.div
          ...
          className="relative w-full max-w-3xl rounded-3xl bg-white shadow-2xl border border-neutral-200/90 z-10 overflow-hidden my-8 max-h-[90vh] flex flex-col"
        >
          {/* Header Bar */}
              ...
              <div className="flex rounded-full bg-neutral-200/80 p-0.5 text-xs font-bold">
                <button
                  ...
                  className={`px-3 py-1 rounded-full transition-all cursor-pointer ${
                    activeTab === 'form' ? 'bg-white text-neutral-900 shadow-xs' : 'text-neutral-500 hover:text-neutral-800'
                  }`}
                >
                  편집 폼
                </button>
          ...
          {activeTab === 'form' ? (
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 sm:p-8 space-y-6">
              ...
              <div>
                <label className="block text-xs font-bold tracking-wider text-neutral-700 uppercase mb-2">
                  1. 작업 카테고리 선택 (카테고리에 따라 양식과 AI 작성이 자동 최적화됩니다) *
                </label>
                <div className="grid grid-cols-3 gap-3">
                  {[
                    { id: 'YOUTUBE VIDEO', label: '유튜브 롱폼', sub: 'Long-form Video', icon: Film, color: 'blue' },
                    { id: 'SHORTS / REELS', label: '숏폼 / 릴스', sub: 'Vertical 9:16', icon: Play, color: 'purple' },
                    { id: 'PRODUCT PAGE', label: '상세페이지 LP', sub: 'e-Commerce LP', icon: ShoppingBag, color: 'emerald' },
                  ].map((cat) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-[10px] text-neutral-400 mt-0.5">{cat.sub}</span>
                      </button>
                    );
                  })}
                </div>
              ...
              <div className="rounded-2xl border border-neutral-200/80 bg-neutral-50/40 p-4 sm:p-5 space-y-4">
                <div className="flex items-center gap-2 border-b border-neutral-200/60 pb-2.5">
                  <Tag size={15} className={isProductPage ? 'text-emerald-600' : 'text-blue-600'} />
                  <span className="text-xs font-bold text-neutral-800 uppercase tracking-wider">
                    {isProductPage ? '2. 상세페이지 기본 정보' : '2. 영상 프로젝트 기본 정보'}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="sm:col-span-2">
                    <label className="block text-xs font-bold tracking-wider text-neutral-700 uppercase mb-1.5">
                      {isProductPage ? '상품명 / 상세페이지 프로젝트 제목 *' : '영상 프로젝트 제목 *'}
                    </label>
                    <input
                      type="text"
                      required
                  ...
                  <div>
                    <label className="block text-xs font-bold tracking-wider text-neutral-700 uppercase mb-1.5">
                      {isProductPage ? '브랜드명 / 고객사 (Brand / Client)' : '클라이언트 / 유튜브 채널명'}
                    </label>
                    <input
                      type="text"
                      value={client}
                      onChange={(e) => setClient(e.target.value)}
                      placeholder={isProductPage ? '예: 퓨어랩스 (PureLabs)' : '예: IT/테크 크리에이터 채널'}
                      className="w-full rounded-xl border border-neutral-200 bg-white px-4 py-2.5 text-sm font-medium text-neutral-900 focus:border-blue-600 focus:outline-none transition shadow-2xs"
                    />
                  </div>
                  ...
                  <div>
                    <label className="block text-xs font-bold tracking-wider text-neutral-700 uppercase mb-1.5">
                      {isProductPage ? '상세페이지 규격 / 길이 (Spec / Height)' : '영상 분량 / 길이 (Duration)'}
                    </label>
                    <input
                      type="text"
                      value={duration}
              ...
              <div className="rounded-2xl border border-neutral-200/80 bg-neutral-50/40 p-4 sm:p-5 space-y-4">
                <div className="flex items-center gap-2 border-b border-neutral-200/60 pb-2.5">
                  <ImageIcon size={15} className={isProductPage ? 'text-emerald-600' : 'text-blue-600'} />
                  <span className="text-xs font-bold text-neutral-800 uppercase tracking-wider">
                    {isProductPage ? '3. 상세페이지 대표 이미지 및 뷰어 에셋' : '3. 썸네일 및 동영상 링크'}
                  </span>
                </div>
                ...
                <div>
                  <label className="block text-xs font-bold tracking-wider text-neutral-700 uppercase mb-1.5">
                    {isProductPage ? '대표 썸네일 / 히어로 메인 이미지 URL *' : '대표 썸네일 이미지 URL *'}
                  </label>
                  <input
                    type="url"
                    required
                  ...
                  <div className="flex items-center gap-1.5 overflow-x-auto pb-1 no-scrollbar text-xs">
                    <span className="text-[11px] text-neutral-400 font-bold shrink-0">추천 프리셋:</span>
                    {(isProductPage ? PRESET_PRODUCT_IMAGES : PRESET_VIDEO_IMAGES).map((preset, idx) => (
                      <button
                        type="button"
                        key={idx}
                ...
                {/* Video URL & YouTube Integration (Only for Video Projects) */}
                {!isProductPage && (
                  <div className="space-y-3 pt-1 border-t border-neutral-200/60">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5">
                      <label className="flex items-center gap-1.5 text-xs font-bold tracking-wider text-neutral-800 uppercase">
                        <Video size={14} className="text-red-600" />
                        <span>동영상 링크 (YouTube 영상 / 쇼츠 / MP4) *</span>
                      </label>
                      ...
                      {isYouTubeUrl(videoUrl) && (
                        <button
                          ...
                          className="self-start sm:self-auto inline-flex items-center gap-1 text-[11px] font-bold text-red-600 bg-red-50 hover:bg-red-100 border border-red-200 px-2.5 py-1 rounded-lg transition cursor-pointer active:scale-95"
                          title="YouTube 고화질 썸네일을 대표 이미지로 적용합니다"
                        >
                          <Sparkles size={12} className="text-red-600" />
                          <span>유튜브 썸네일 대표 이미지로 자동 가져오기</span>
                        </button>
                      )}
                    </div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setVideoUrl(val);
                          // If user pastes a valid YouTube URL and current image is default preset, auto-suggest thumbnail
                          if (isYouTubeUrl(val)) {
                            const isDefaultImg = PRESET_VIDEO_IMAGES.some(p => p.url === image);
                            if (isDefaultImg || !image) {
                        ...
                        placeholder={
                          isShorts
                            ? 'https://youtube.com/shorts/VIDEO_ID 또는 https://youtu.be/...'
                            : 'https://www.youtube.com/watch?v=VIDEO_ID 또는 https://youtu.be/...'
                        }
                        className="w-full rounded-xl border border-neutral-200 bg-white px-4 py-2.5 text-sm font-medium text-neutral-900 focus:border-red-600 focus:outline-none transition shadow-2xs pr-24"
                      />
                    ...
                    {/* YouTube Helper & Quick Samples */}
                    <div className="flex flex-wrap items-center gap-1.5 text-xs">
                      <span className="text-[11px] text-neutral-400 font-bold shrink-0">빠른 샘플 채우기:</span>
                      <button
                        type="button"
                        onClick={() => {
                          const sample = 'https://www.youtube.com/watch?v=aqz-KE-bpKQ';
                          setVideoUrl(sample);
                          const thumb = getYouTubeThumbnailUrl(sample, 'maxres') || getYouTubeThumbnailUrl(sample, 'hq');
                          if (thumb) setImage(thumb);
                        }}
                        className="px-2 py-0.5 rounded-md bg-white border border-neutral-200 hover:bg-neutral-50 text-neutral-600 text-[10px] font-medium transition cursor-pointer"
                      >
                        유튜브 롱폼 샘플
                      </button>
                      <button
                        type="button"
                        onClick={() => {
                          const sample = 'https://youtube.com/shorts/4q1Z0eO3q4s';
                          setVideoUrl(sample);
                          const thumb = getYouTubeThumbnailUrl(sample, 'hq');
                          if (thumb) setImage(thumb);
                        }}
                        className="px-2 py-0.5 rounded-md bg-white border border-neutral-200 hover:bg-neutral-50 text-neutral-600 text-[10px] font-medium transition cursor-pointer"
                      >
                    ...
                    {/* In-form Mini Preview for YouTube Video */}
                    {isYouTubeUrl(videoUrl) && (
                      <div className="mt-2 p-3 rounded-xl bg-neutral-900 text-white space-y-2">
                        <div className="flex items-center justify-between text-[11px] text-neutral-300">
                          <span className="font-bold flex items-center gap-1.5 text-red-400">
                            <Video size={13} />
                            유튜브 비디오 연동 확인 (Video ID: {extractYouTubeId(videoUrl)})
                          </span>
                          <a
                            href={videoUrl}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="text-neutral-400 hover:text-white flex items-center gap-1 text-[10px] underline"
                          >
                            <span>YouTube에서 확인</span>
                            <ExternalLink size={10} />
                          </a>
                        </div>
                        <div className={`relative ${isShorts ? 'max-w-xs mx-auto aspect-9/16' : 'aspect-16/9'} w-full rounded-lg overflow-hidden bg-black`}>
                          <iframe
                            src={getYouTubeEmbedUrl(videoUrl, { autoplay: false, mute: true }) || ''}
                  ...
                  <>
                    <div>
                      ...
                      <p className="text-[11px] text-neutral-400 mt-1">
                        * 비워둘 경우 기본 섹션 뷰어 또는 대표 썸네일로 자동 구성됩니다.
                      </p>
                    </div>

                    <div>
                      <label className="block text-xs font-bold tracking-wider text-neutral-700 uppercase mb-1.5">
                        실제 판매처 / 펀딩 링크 (선택 사항 - 스마트스토어/쿠팡/와디즈)
                      </label>
                    ...
                    <div className="pt-2 border-t border-neutral-200/60">
                      <div className="flex items-center justify-between">
                        ...
                        >
                          ...
                          <span>상세페이지 단락별 섹션 블록 빌더 ({detailSections.length}개)</span>
                          {showSectionBuilder ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                        </button>
                        ...
                        <div className="mt-3 space-y-3">
                          {detailSections.length === 0 ? (
                            <div className="text-center p-4 rounded-xl bg-white border border-dashed border-neutral-200 text-xs text-neutral-400">
                              등록된 세부 섹션이 없습니다. [섹션 단락 추가] 버튼을 눌러 인트로, 특장점, 스펙 단락을 빌드해보세요.
                            </div>
                          ) : (
                            detailSections.map((sec, idx) => (
                              <div key={idx} className="p-3 rounded-xl bg-white border border-neutral-200 shadow-2xs space-y-2 text-xs">
                                <div className="flex items-center justify-between border-b border-neutral-100 pb-1.5">
                                  <span className="font-bold text-emerald-800">섹션 0{idx + 1}</span>
                                  <button
                                    type="button"
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                  <input
                                    type="text"
                                    value={sec.badge || ''}
                                    onChange={(e) => handleSectionFieldChange(idx, 'badge', e.target.value)}
                                ...
                                <input
                                  type="text"
                                  value={sec.image || ''}
                                  onChange={(e) => handleSectionFieldChange(idx, 'image', e.target.value)}
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 mb-3">
                      ...
                      >
                        ...
                        <span
                          className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${
                            isProductPage ? 'bg-emerald-100 text-emerald-700' : 'bg-indigo-100 text-indigo-700'
                          }`}
                        >
                          {isProductPage ? 'e-Commerce MD AI' : 'Video Director AI'}
                        </span>
                      </h4>
                  ...
                  <button
                    ...
                    className={`inline-flex items-center justify-center gap-1.5 px-3.5 py-2 rounded-xl text-white text-xs font-bold shadow-xs transition cursor-pointer disabled:opacity-50 shrink-0 ${
                      isProductPage ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-indigo-600 hover:bg-indigo-700'
                    }`}
                  >
                    {isGeneratingAI ? (
                      <>
                      ...
                      <>
                        <Wand2 size={13} />
                        <span>{isProductPage ? '상세페이지 카피 일괄 추천' : '전체 문구 일괄 추천'}</span>
                      </>
                    )}
                  </button>
                </div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className={`flex-1 rounded-xl border bg-white px-3.5 py-2 text-xs font-medium text-neutral-900 placeholder:text-neutral-400 focus:outline-none transition shadow-2xs ${
                      isProductPage ? 'border-emerald-200 focus:border-emerald-600' : 'border-indigo-200 focus:border-indigo-600'
                    }`}
                  />
                  <button
                    type="button"
                ...
                <div className="flex items-center gap-1.5 overflow-x-auto pt-2 no-scrollbar text-xs">
                  <span className="text-[10px] text-neutral-400 font-bold shrink-0">빠른 키워드:</span>
                  {(isProductPage
                    ? [
                ...
                {aiSuccessMsg && (
                  <div
                    className={`mt-2.5 flex items-center justify-between text-xs font-semibold px-3 py-1.5 rounded-lg ${
                      isProductPage ? 'text-emerald-800 bg-emerald-100/80' : 'text-indigo-800 bg-indigo-100/80'
                    }`}
                  >
                ...
                {aiGeneratedResult && showAiAssistant && (
                  <div className="mt-3 pt-3 border-t border-neutral-200/80 space-y-2.5">
                    <div className="flex items-center justify-between">
                      ...
                      >
                        {isProductPage ? '🛍️ 상세페이지 AI 맞춤 추천 결과' : '📹 영상 편집 AI 추천 결과'}
                      </span>
                      <button
                        type="button"
                      ...
                      <div className="p-2.5 rounded-xl bg-white/90 border border-neutral-200 text-xs text-neutral-700 space-y-1">
                        <div className="flex items-center justify-between text-[11px] font-bold text-neutral-500">
                          ...
                          <button
                            ...
                            className={isProductPage ? 'text-emerald-600 hover:underline cursor-pointer' : 'text-indigo-600 hover:underline cursor-pointer'}
                          >
                            적용
                          </button>
                        </div>
                        <p className="line-clamp-2">{aiGeneratedResult.description}</p>
                      </div>
                    )}
                    ...
                    {aiGeneratedResult.fullStory && (
                      <div className="p-2.5 rounded-xl bg-white/90 border border-neutral-200 text-xs text-neutral-700 space-y-1">
                        ...
                        <p className="line-clamp-4 whitespace-pre-line text-[11px] text-neutral-600">{aiGeneratedResult.fullStory}</p>
                      </div>
                    )}
                  </div>
                )}
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label className="block text-xs font-bold tracking-wider text-neutral-700 uppercase">
                    {isProductPage ? '카드 요약 카피라이팅 (1~2문장) *' : '카드 요약 설명 *'}
                  </label>
                  <button
                    type="button"
                    ...
                    className={`text-[11px] font-bold flex items-center gap-1 transition cursor-pointer ${
                      isProductPage ? 'text-emerald-700 hover:text-emerald-900' : 'text-indigo-600 hover:text-indigo-800'
                    }`}
                  >
                    <Sparkles size={12} />
                    <span>{isProductPage ? 'AI 상세페이지 요약만 생성' : 'AI 요약 문구만 생성'}</span>
                  </button>
                </div>
                <textarea
                  required
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label className="block text-xs font-bold tracking-wider text-neutral-700 uppercase">
                    {isProductPage ? '상세 기획 & 디자인 스토리 (모달 상세페이지용)' : '상세 작업 스토리 (모달 상세페이지용)'}
                  </label>
                  <button
                    type="button"
                  ...
                  >
                    <Sparkles size={12} />
                    <span>{isProductPage ? 'AI 상세 스토리만 생성' : 'AI 스토리만 생성'}</span>
                  </button>
                </div>
                <textarea
                  rows={5}
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="block text-xs font-bold tracking-wider text-neutral-700 uppercase">
                    {isProductPage ? '상세페이지 핵심 작업 포인트 (Highlights)' : '핵심 작업 포인트 (Highlights)'}
                  </label>
                  <button
                    type="button"
                ...
                <div className="space-y-2">
                  {highlights.map((highlight, index) => (
                    <div key={index} className="flex items-center gap-2">
                      <span className="text-xs font-bold text-neutral-400 w-4 text-center">{index + 1}</span>
                      <input
                        type="text"
                        value={highlight}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2 border-t border-neutral-100">
                <div>
                  <label className="block text-xs font-bold tracking-wider text-neutral-700 uppercase mb-2">
                    {isProductPage ? '사용 디자인 툴 태그 (Tools)' : '사용 툴 태그 (Tools)'}
                  </label>
                  <div className="flex gap-2 mb-2">
                    <input
                      ...
                      placeholder={isProductPage ? '예: Photoshop, Figma' : '예: Premiere Pro, AE'}
                ...
                <div className="flex flex-col justify-center">
                  ...
                  <label className="flex items-center gap-3 p-3 rounded-xl border border-neutral-200 bg-neutral-50/60 cursor-pointer hover:bg-neutral-50">
                    ...
                    />
                    <div>
                      <span className="text-xs font-bold text-neutral-900 block">홈(Home) 큐레이션 노출</span>
                      <span className="text-[11px] text-neutral-500 block">메인 홈 상위 4개 큐레이션 작업물에 포함</span>
                    </div>
                  </label>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 sm:p-8 flex flex-col items-center justify-center bg-neutral-50/50">
              <div className="w-full max-w-sm rounded-2xl border border-neutral-200/80 bg-white p-4 shadow-md flex flex-col justify-between">
                <div>
                  <div className={`relative ${isShorts ? 'aspect-9/16 max-w-[220px] mx-auto' : 'aspect-16/10'} w-full overflow-hidden rounded-xl bg-neutral-950`}>
                    <img
                      src={image || (isProductPage ? PRESET_PRODUCT_IMAGES[0].url : PRESET_VIDEO_IMAGES[0].url)}
                      alt={title || '미리보기'}
                    ...
                    <div className="absolute top-3 left-3 z-10 flex gap-1.5 flex-wrap">
                      <span
                        className={`rounded-md px-2.5 py-1 text-[10px] font-bold tracking-wider text-white uppercase backdrop-blur-xs ${
                          isProductPage ? 'bg-emerald-950/80' : isShorts ? 'bg-red-900/85' : 'bg-neutral-950/75'
                        }`}
                      >
                        {category}
                      </span>
                      ...
                      {isYouTubeUrl(videoUrl) && (
                        <span className="rounded-md bg-red-600 text-white px-2 py-1 text-[9px] font-bold shadow-xs flex items-center gap-1">
                          <Video size={10} />
                          YouTube
                        </span>
                      )}
                  ...
                  <div className="mt-4">
                    <div className="flex items-center justify-between text-[11px] font-medium text-neutral-400">
                      <span>{client || (isProductPage ? '브랜드사' : '고객사 명')}</span>
                      <span>{year || '2024'}</span>
                    </div>
                ...
                <div className="mt-4 pt-3 border-t border-neutral-100 flex items-center justify-between text-[11px]">
                  <span className={`font-semibold ${isProductPage ? 'text-emerald-600' : isShorts ? 'text-red-600' : 'text-blue-600'}`}>
                    {isProductPage ? 'VIEW DETAIL LP →' : 'VIEW PROJECT →'}
                  </span>
                  <span className="text-neutral-400">{duration || (isProductPage ? '860px LP' : '소요 분량')}</span>
                </div>
              </div>
