...
import { isYouTubeUrl, getYouTubeEmbedUrl, extractYouTubeId } from '../utils/youtube';
import {
  X,
  Play,
  Pause,
          ...
          <div className="p-4 sm:p-6 md:p-8 space-y-6 sm:space-y-8">
            ...
            {activeTab === 'video' && isVideoProject && (
              <div className="space-y-4">
                ...
                {/* Dedicated Video Player (YouTube iframe or HTML5 Video) */}
                {isYouTubeUrl(project.videoUrl) ? (
                  <div className="space-y-3">
                    <div
                      className={`relative w-full ${
                        project.categoryTag === 'SHORTS' || project.category === 'SHORTS / REELS'
                          ? 'max-w-xs sm:max-w-sm mx-auto aspect-9/16'
                          : 'aspect-16/9'
                      } bg-neutral-950 rounded-2xl overflow-hidden shadow-xl border border-neutral-800`}
                    >
                      {/* Copyright Watermark Protection Overlay */}
                      <WatermarkOverlay variant="video" text="lovey" />

                      <iframe
                        src={getYouTubeEmbedUrl(project.videoUrl, { autoplay: true, mute: isMuted, loop: true }) || ''}
                        title={projectTitle}
                    ...
                    {/* YouTube Extra Meta & External Link Bar */}
                    <div className="flex items-center justify-between text-xs text-neutral-500 bg-neutral-50 px-4 py-2.5 rounded-xl border border-neutral-200/70">
                      <div className="flex items-center gap-2">
                        <span className="inline-flex h-2 w-2 rounded-full bg-red-600 animate-pulse" />
                        <span className="font-bold text-neutral-800">YouTube 영상 스트리밍</span>
                        <span className="text-neutral-400">·</span>
                        <span className="font-mono text-[11px] text-neutral-500">ID: {extractYouTubeId(project.videoUrl)}</span>
                      </div>
                      <a
                        href={project.videoUrl}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-1.5 text-xs font-bold text-red-600 hover:text-red-700 transition hover:underline"
                      >
